...
export class SmartContractUtil {
    ...
    public static async getConnectionProfile() {
        ...
        const connectionProfilePath: string = path.join(homedir, 'FinanceNetworks', 'gateways', 'FI1', 'FI1 gateway.json');
    ...
    public static async submitTransaction(contractName: string, functionName: string, args: string[], gateway): Promise<Buffer> {
        // Submit transaction
        const network: fabricNetwork.Network = await gateway.getNetwork('financechannel');
        let contract: fabricNetwork.Contract;
        if (contractName !== '') {
            contract = await network.getContract('eKYC', contractName);
        } else {
            contract = await network.getContract('eKYC');
        }
