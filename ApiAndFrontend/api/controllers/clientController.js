...
exports.getNotiFications= async (req,res) => {
 ...
 console.log("Email", email)
    io.getNotificaion("",email).then(result =>{
            console.log(result)
            io.updateNotification(email);
...
exports.getClientData = (req, res) => {
    ...
    const fields = ['name', 'address', 'dateOfBirth', 'idNumber', 'whoRegistered','email'];

    networkConnection
        .evaluateTransaction('getClientData', "fi"+req.orgNum, req.ledgerUser, [req.cookies.ledgerId, fields || []])
        .then(result => {
            if (result) {
                if (result.length > 0) {
...
exports.approve = async (req, res) => {
    ...
    networkConnection
        .submitTransaction('approve', "fi"+req.orgNum, req.ledgerUser, [req.cookies.ledgerId, fiId])
        .then(result => {
            if (result) {
...
exports.remove = async (req, res) => {
    ...
    networkConnection
        .submitTransaction('remove', "fi"+req.orgNum, req.ledgerUser, [req.cookies.ledgerId, fiId])
        .then(result => {
            if (result) {
...
exports.getApprovedFis = async (req, res) => {
    networkConnection
        .evaluateTransaction('getRelationByClient', "fi"+req.orgNum, req.ledgerUser, [req.cookies.ledgerId])
        .then(result => {
            if (result) {
                if (result.length > 0) {
