...
const Client = () => {
    ...
    function getMessages(email){
        api
        .post('/client/getAllNotification', qs.stringify({"email":email}) )
        .then(res => {
            let data ='';
            
            console.log(res, res.leghth )
            if(res && res.data.length > 0){
                setIsOpen(true);
                for(var i =0; i < res.data.length ;i++){
                    data = data + res.data[i].subject +"  <br>";
                }
            }
            setMessageData(data)
            console.log(data)
    ...
    return (
        <Flex minWidth={380}>
          ...
          <Card width={"420px"} p={0}>
            ...
            <Box p={4} mb={3}>
              <Heading.h3>Access Request</Heading.h3>
              <Text>  {messageData}
              </Text>
            </Box>
